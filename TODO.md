...
[x] Product slug collision risk - strings.ToLower(strings.ReplaceAll(name, " ", "-")) with no uniqueness check
[x] In-memory session store - sessions lost on restart, no revocation, not scalable, vulnerable to session fixation
[x] OTP codes never cleaned up - otp_codes table grows indefinitely
[x] No graceful shutdown - HTTP server doesn't handle SIGTERM, in-flight requests dropped
